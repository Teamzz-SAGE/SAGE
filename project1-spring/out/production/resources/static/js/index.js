...
// 체크박스 값 → 데이터 필드명 매핑 (Facility DTO 필드명과 꼭 일치!)
const PROGRAM_FIELD_MAP = {
  art: 'programArt',
  rest: 'programLeisure',     // ★ 'rest'는 DB/DTO에서 'programLeisure' 임
  dementia: 'programDementia',
  religion: 'programReligion',
  cognitive: 'programCognition',
  physical: 'programPhysical',
  therapy: 'programTherapy',
  beauty: 'programBeauty',
  specialization: 'programSpecial',
  etc: 'programEtc'
};

// 다양한 표기(Y/1/예/운영/true 등) 허용
function isTruthy(v) {
  if (v === true) return true;
  if (typeof v === 'number') return v !== 0;
  if (!v) return false;
  const s = String(v).trim().toLowerCase();
  return ['y','yes','true','1','예','가능','o','운영'].includes(s);
}

function getSelectedPrograms() {
  return Array.from(
      document.querySelectorAll('input[type="checkbox"][data-filter-type="program"]:checked')
  ).map(cb => cb.dataset.filterValue); // 예: ["art","rest","therapy"]
}

function hasProgram(facility, key) {
  // 배열형 프로그램이 있다면 여기서 처리 (지금은 개별 필드 기반)
  const field = PROGRAM_FIELD_MAP[key];
  if (!field) return false;
  return isTruthy(facility[field]);
}

// mode: 'AND' | 'OR'
function matchPrograms(facility, selectedKeys, mode = 'AND') {
  if (!selectedKeys.length) return true;      // 선택 없으면 통과
  if (mode === 'OR')  return selectedKeys.some(k => hasProgram(facility, k));
  return selectedKeys.every(k => hasProgram(facility, k)); // 기본 AND
}

function applyProgramFilters(mode = 'AND') {
  if (!initialFacilityData || initialFacilityData.length === 0) {
    console.warn('initialFacilityData 비어있음. 데이터 로딩 확인');
    return;
  }
  const selected = getSelectedPrograms();
  const filtered = initialFacilityData.filter(f => matchPrograms(f, selected, mode));
  indexRenderFacilitiesTablePaged(filtered, 1, PER_PAGE);
}
...
function indexRenderFacilitiesList(list) {
  ...
  list.forEach((fac) => {
    ...
    const imageUrl = fac.mainPhoto || '/img/default.jpg'; // 'image' 속성 또는 'imageUrl'을 대체로 사용합니다.

          container.innerHTML += `
        <li class="facility-item" style="cursor: pointer;" onclick="window.location.href='/facility-detail/${id}'">
          <img src="${imageUrl}" alt="시설사진" class="facility-thumb" onerror="this.src='/img/default.jpg';">
...
function showCompareSidebar() {
  ...
  if (sidebar) {
    ...
    document.querySelector('.facility-main').classList.add('sidebar-open');
  }
  // 사이드바가 열릴 때 접힌 버튼 숨기기
  if (collapsedBtn) {
    collapsedBtn.style.display = 'none';
  }
  console.log('사이드바 열기:', sidebar ? sidebar.style.display : '사이드바 없음');
}
...
function hideCompareSidebar() {
  ...
  if (sidebar) {
    ...
    document.querySelector('.facility-main').classList.remove('sidebar-open');
  }
  ...
  if (collapsedBtn && heartSelections.length > 0) {
    ...
    collapsedBtn.style.top = '50%';
    collapsedBtn.style.right = '20px';
    collapsedBtn.style.left = 'auto';
    collapsedBtn.style.transform = 'translateY(-50%)';
  }
  console.log('사이드바 닫기, 비교 버튼 표시:', collapsedBtn ? collapsedBtn.style.display : '버튼 없음');
}
...
function updateCompareSidebar() {
  ...
  // 사이드바는 기본적으로 닫혀있고, 비교 버튼만 표시
  const sidebar = document.getElementById('compareSidebar');
  const collapsedBtn = document.getElementById('collapsedSidebarBtn');

  if (heartSelections.length > 0) {
    // 선택된 항목이 있으면 사이드바는 닫혀있고 비교 버튼만 표시
    if (sidebar) {
      sidebar.style.display = 'none';
      document.querySelector('.facility-main').classList.remove('sidebar-open');
    }
...
function updateCompareButton() {
  ...
  if (collapsedSidebarBtn) {
    const sidebar = document.getElementById('compareSidebar');
    const isSidebarOpen = sidebar && (sidebar.style.display === 'flex' || sidebar.style.display === 'block');
    ...
    if (heartSelections.length > 0) {
      if (isSidebarOpen) {
        ...
        // 사이드바가 닫혀있으면 버튼 표시 (오른쪽 중앙)
        collapsedSidebarBtn.style.display = 'flex';
        collapsedSidebarBtn.style.position = 'fixed';
        collapsedSidebarBtn.style.top = '50%';
        collapsedSidebarBtn.style.right = '20px';
        collapsedSidebarBtn.style.left = 'auto';
        collapsedSidebarBtn.style.transform = 'translateY(-50%)';
      }
    } else {
      // 선택된 항목이 없으면 버튼 숨김
      collapsedSidebarBtn.style.display = 'none';
...
function initializeFacilityData(data) {
  ...
  const facilityCountSpan = document.querySelector('.facility-count');
  if (facilityCountSpan) {
    facilityCountSpan.textContent = data.length + '개';
  }

  const anyProgramChecked = document.querySelector('input[type="checkbox"][data-filter-type="program"]:checked');
  if (anyProgramChecked) {
    applyProgramFilters('AND');
  } else {
    indexRenderFacilitiesTablePaged(initialFacilityData, 1, PER_PAGE);
  }


  // 초기 상태 설정: 사이드바는 닫혀있고, 비교 버튼만 표시
  const sidebar = document.getElementById('compareSidebar');
  const collapsedBtn = document.getElementById('collapsedSidebarBtn');
  ...
  setTimeout(() => {
    ...
    // 비교 시설이 있으면 비교 버튼 표시 (오른쪽 중앙)
    const heartSelections = JSON.parse(localStorage.getItem('heartSelections') || '[]');
    if (heartSelections.length > 0 && collapsedBtn) {
      collapsedBtn.style.display = 'flex';
      collapsedBtn.style.position = 'fixed';
      collapsedBtn.style.top = '50%';
      collapsedBtn.style.right = '20px';
      collapsedBtn.style.left = 'auto';
      collapsedBtn.style.transform = 'translateY(-50%)';
      console.log('초기화: 비교 버튼 표시됨');
    } else {
      console.log('초기화: 비교 시설 없음 또는 버튼 없음');
    }
  }, 100);
}
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  sortBtns.forEach(btn => {
    ...
    document.addEventListener('change', (e) => {
      if (e.target.matches('input[type="checkbox"][data-filter-type="program"]')) {
        applyProgramFilters('AND'); // OR 로 바꾸고 싶으면 'OR'
      }
    });
  ...
  function highlightAndFilterRegion(regionKor) {
    ...
    const facilityCountSpan = document.querySelector('.facility-count');
    if (facilityCountSpan) {
      facilityCountSpan.textContent = filtered.length + '개';
    }
  ...
  if (closeSidebarBtn) {
    closeSidebarBtn.addEventListener('click', function() {
      hideCompareSidebar();
      updateCompareButton(); // 버튼 상태 업데이트
    });
  }
